...
// (0x7ffffff)
qj=Math.min(qj/bhi|0,67108863);a._ishlnsubmul(b,qj,j);while(a.negative!==0){qj--;a.negative=0;a._ishlnsubmul(b,1,j);if(!a.isZero()){a.negative^=1;}}if(q){q.words[j]=qj;}}if(q){q._strip();}a._strip();// Denormalize
...
return dm.div.negative!==0?dm.div.isubn(1):dm.div.iaddn(1)};BN.prototype.modrn=function modrn(num){var isNegNum=num<0;if(isNegNum)num=-num;assert(num<=67108863);var p=(1<<26)%num;var acc=0;for(var i=this.length-1;i>=0;i--){acc=(p*acc+(this.words[i]|0))%num;}return isNegNum?-acc:acc};// WARNING: DEPRECATED
...
var mask=4194303;var outLen=Math.min(input.length,9);for(var i=0;i<outLen;i++){output.words[i]=input.words[i];}output.length=outLen;if(input.length<=9){input.words[0]=0;input.length=1;return}// Shift by 9 limbs
...
var lo=0;for(var i=0;i<num.length;i++){var w=num.words[i]|0;lo+=w*977;num.words[i]=lo&67108863;lo=w*64+(lo/67108864|0);}// Fast length reduction
...
var carry=0;for(var i=0;i<num.length;i++){var hi=(num.words[i]|0)*19+carry;var lo=hi&67108863;hi>>>=26;num.words[i]=lo;carry=hi;}if(carry!==0){num.words[num.length++]=carry;}return num};// Exported mostly for testing purposes, use plain name instead
...
var res=this.imod(a._invmp(this.m).mul(this.r2));return res._forceRed(this)};})(module,commonjsGlobal);})(bn);var BigNumber = bn.exports;

const bitsFromLeft=(value,totalBits)=>{const extractor=bits=>{const extracted=extractor.peek(bits);extractor.bitsLeft-=bits;return extracted};extractor.peek=bits=>extractor.bitsLeft>bits?value.shrn(extractor.bitsLeft-bits).maskn(bits):value.maskn(bits);extractor.bitsLeft=totalBits;return extractor};const getAxieBodyStructure512=geneString=>{const genes=new BigNumber(geneString.replace("0x",""),16);const bits=bitsFromLeft(genes,512);const mainClass=bits(5).toNumber();bits(45).toNumber();bits(5).toNumber();bits(1).toNumber();const bodySkin=bits(9).toNumber();const bodyDetail0=bits(9).toNumber();const bodyDetail1=bits(9).toNumber();const bodyDetail2=bits(9).toNumber();const primaryColor0=bits(6).toNumber();const primaryColor1=bits(6).toNumber();const primaryColor2=bits(6).toNumber();const secondaryColor0=bits(6).toNumber();const secondaryColor1=bits(6).toNumber();const secondaryColor2=bits(6).toNumber();const bodyStructure={class:getCharacterClassFromValue(mainClass),body:[bodyDetail0,bodyDetail1,bodyDetail2],bodySkin:bodySkin,primaryColors:[primaryColor0,primaryColor1,primaryColor2],secondaryColors:[secondaryColor0,secondaryColor1,secondaryColor2],parts:{}};for(let partIndex=0;partIndex<6;partIndex++){const partReservation=bits(12).toNumber();const partStage=bits(3).toNumber();const partSkinInheritability=bits(1).toNumber();const partSkin=bits(9).toNumber();const partClass0=bits(5).toNumber();const partValue0=bits(8).toNumber();const partClass1=bits(5).toNumber();const partValue1=bits(8).toNumber();const partClass2=bits(5).toNumber();const partValue2=bits(8).toNumber();const partType=axiePartTypes[partIndex];const part={stageCap:2,stage:partStage,reservation:partReservation,skinInheritability:partSkinInheritability===0?false:true,skin:partSkin,groups:[]};part.groups.push({class:getCharacterClassFromValue(partClass0),value:partValue0});part.groups.push({class:getCharacterClassFromValue(partClass1),value:partValue1});part.groups.push({class:getCharacterClassFromValue(partClass2),value:partValue2});bodyStructure.parts[partType]=part;}return bodyStructure};

function capitalizeFirstLetter(string){return string.charAt(0).toUpperCase()+string.slice(1)}class AxieGenesStuff{// eslint-disable-next-line @typescript-eslint/no-empty-function
constructor(){}load(genesJson){const genesData=genesJson.items;const genesColor=genesData.colors;const genesBodies=genesData.bodies;const genesParts=genesData.parts;this.bodySamples=[];for(const genesBody of genesBodies){const bodySample={skin:genesBody["skin"],bodyValue:genesBody["bodyValue"],mysticValue:genesBody["mysticValue"],bodyName:genesBody["bodyName"]};this.bodySamples.push(bodySample);}this.partSamples=[];for(const genesPart of genesParts){const skins=[];for(const skinName of genesPart["skins"]){skins.push(skinName);}const skinsLv2=[];for(const skinName of genesPart["skinsLv2"]){skinsLv2.push(skinName);}const partSample={class:capitalizeFirstLetter(genesPart["class"]),partType:capitalizeFirstLetter(genesPart["partType"]),partValue:genesPart["partValue"],skins:skins,skinsLv2:skinsLv2};this.partSamples.push(partSample);}this.axieSkinColors=[];for(const axieSkinColor of genesColor){const sampleColor={key:axieSkinColor["key"],skin:axieSkinColor["skin"],class:capitalizeFirstLetter(axieSkinColor["class"]),colorValue:axieSkinColor["color_value"],primary1:axieSkinColor["primary1"],shaded1:axieSkinColor["shaded1"],primary2:axieSkinColor["primary2"],shaded2:axieSkinColor["shaded2"],line:axieSkinColor["line"],partColorShift:axieSkinColor["partColorShift"]};this.axieSkinColors.push(sampleColor);}}getBodySample(value,skin,isMystic){let bodyIndex=-1;for(let i=0;i<this.bodySamples.length;i++){const bodySample=this.bodySamples[i];if(bodySample.skin!=skin){continue}if(bodySample.mysticValue!=-1&&bodySample.mysticValue!=(isMystic?1:0)){continue}if(bodySample.bodyValue!=-1&&bodySample.bodyValue!=value){continue}bodyIndex=i;break}if(bodyIndex===-1){bodyIndex=0;}return this.bodySamples[bodyIndex].bodyName}findPart(partClass,partType,partValue){const partIndex=this.partSamples.findIndex(function(x){return x.class===partClass&&x.partType===partType&&x.partValue===partValue});if(partIndex===-1){return null}return this.partSamples[partIndex]}findPartBySample(partType,sampleName){const partIndex=this.partSamples.findIndex(function(x){return x.partType===partType&&(x.skins.indexOf(sampleName)!=-1||x.skinsLv2.indexOf(sampleName)!=-1)});if(partIndex===-1){return null}return this.partSamples[partIndex]}overridePartSample(partType,sampleName,stage,skin){if(stage==-1&&skin==-1){return sampleName}for(let i=0;i<this.partSamples.length;i++){const p=this.partSamples[i];if(p.partType!==partType){continue}const lv1Skin=p.skins.indexOf(sampleName);const lv2Skin=p.skinsLv2.indexOf(sampleName);if(lv1Skin!=-1||lv2Skin!=-1){let finalStage=stage;if(finalStage==-1){finalStage=lv1Skin!=-1?0:1;}let finalSkin=skin;if(finalSkin==-1){finalSkin=lv1Skin!=-1?lv1Skin:lv2Skin;}if(finalStage==0){return p.skins[finalSkin].length>0?p.skins[finalSkin]:p.skins[0]}else {return p.skinsLv2[finalSkin].length>0?p.skinsLv2[finalSkin]:p.skinsLv2[0]}}}return sampleName}getPartSample(partType,part){return this.getPartSampleCustom(partType,part,part.stage,part.skin)}getPartSampleCustom(partType,part,stage,skin){const{class:partClass,value:partValue}=part.groups[0];const partSample=this.findPart(partClass,partType,partValue);if(partSample==null){return null}if(stage==1){if(skin>=0&&skin<partSample.skinsLv2.length&&partSample.skinsLv2[skin].length>0){return partSample.skinsLv2[skin]}else {return partSample.skinsLv2[0]}}else {if(skin>=0&&skin<partSample.skins.length&&partSample.skins[skin].length>0){return partSample.skins[skin]}else {return partSample.skins[0]}}}getAdultCombo(bodyStructure){const bodyValue=bodyStructure.body[0];const bodySkin=bodyStructure.bodySkin;let isMystic=false;if(bodySkin==0){for(const partType of axiePartTypes){const part=bodyStructure.parts[partType];if(part.skin===1){isMystic=true;}}}const axieCombo=new Map;axieCombo.set("body",this.getBodySample(bodyValue,bodySkin,isMystic));axieCombo.set("body-class",bodyStructure.class.toString().toLowerCase());axieCombo.set("back",this.getPartSample(exports.AxiePartType.Back,bodyStructure.parts[exports.AxiePartType.Back]));axieCombo.set("ears",this.getPartSample(exports.AxiePartType.Ears,bodyStructure.parts[exports.AxiePartType.Ears]));axieCombo.set("ear",axieCombo.get("ears"));axieCombo.set("eyes",this.getPartSample(exports.AxiePartType.Eyes,bodyStructure.parts[exports.AxiePartType.Eyes]));axieCombo.set("horn",this.getPartSample(exports.AxiePartType.Horn,bodyStructure.parts[exports.AxiePartType.Horn]));axieCombo.set("mouth",this.getPartSample(exports.AxiePartType.Mouth,bodyStructure.parts[exports.AxiePartType.Mouth]));axieCombo.set("tail",this.getPartSample(exports.AxiePartType.Tail,bodyStructure.parts[exports.AxiePartType.Tail]));return axieCombo}getAxieColorsVariant(primaryValue,bodySkin,bodyClass){let variantIndex=-1;for(let i=0;i<this.axieSkinColors.length;i++){const axieSkin=this.axieSkinColors[i];if(axieSkin.skin!=bodySkin){continue}if((axieSkin.class===exports.CharacterClass.Any||axieSkin.class.toLowerCase()===bodyClass.toLowerCase())&&(axieSkin.colorValue===-1||axieSkin.colorValue===primaryValue)){variantIndex=i;break}}if(variantIndex===-1){variantIndex=0;}return variantIndex}getAxieVariantKeyFromIndex(variantIndex){if(variantIndex<0||variantIndex>=this.axieSkinColors.length){return ""}return this.axieSkinColors[variantIndex].key}getAxieColorPartShift(variant){let variantIndex=this.axieSkinColors.findIndex(x=>x.key===variant);if(variantIndex==-1){variantIndex=0;}return this.axieSkinColors[variantIndex].partColorShift}}const genesStuff=/*#__PURE__*/new AxieGenesStuff;

class MixedSkeletonData{constructor(bones,slots,skins,ik,events,animations){this.skeleton={spine:"3.8.79"};this.bones=bones;this.slots=slots;this.skins=skins;this.ik=ik;this.events=events;this.animations=animations;}}

const isBodySample=name=>{for(let i=1;i<boneComboTypes.length;i++){if(name.startsWith(boneComboTypes[i].toString())){return false}}return true};const getSampleName=(name,boneCombo)=>{if(name.length>0&&name[0]==="@"){return boneCombo[0];//body
}for(let i=1;i<boneComboTypes.length;i++){if(name.startsWith(boneComboTypes[i].toString())){return boneCombo[i]}}return boneCombo[0];//body
};const addCustomBoneRule=(edges,sortingRules)=>{if(sortingRules===null){return}for(const p in sortingRules){for(const q of sortingRules[p]){if(p in edges&&q in edges){edges[p].push(q);}}}};const mixEntriesFromSample=(mapEntries,boneCombo,samples,sortingRules)=>{const edges={};const bodyNodes=mapEntries(samples.get(boneCombo[0]));let previousName=bodyNodes[0]["name"];for(let index=1;index<bodyNodes.length;index++){const entry=bodyNodes[index];const entryName=entry["name"];if(!isBodySample(entryName)){continue}if(edges[entryName]){edges[entryName].push(previousName);}else {const vec=[];vec.push(previousName);edges[entryName]=vec;}previousName=entryName;}for(let i=1;i<boneComboTypes.length;i++){const bonePrefix=boneComboTypes[i].toString();const bodyNodes=mapEntries(samples.get(boneCombo[i]));previousName=bodyNodes[0]["name"];for(let index=1;index<bodyNodes.length;index++){const entry=bodyNodes[index];const entryName=entry["name"];if(!isBodySample(entryName)){if(!entryName.startsWith(bonePrefix)){continue}}else if(!entryName.startsWith("body")&&!entryName.startsWith("shadow")){continue}if(edges[entryName]){edges[entryName].push(previousName);}else {const vec=[];vec.push(previousName);edges[entryName]=vec;}previousName=entryName;}}addCustomBoneRule(edges,sortingRules);function visit(name,visited,edges,sortedNames){if(!visited[name]){visited[name]=true;if(edges[name]){for(const edge of edges[name]){visit(edge,visited,edges,sortedNames);}}sortedNames.push(name);}}const visited={};const sortedNames=[];for(const[key,sample]of Array.from(samples.entries())){mapEntries(sample).forEach(entry=>{const entryName=entry["name"];visit(entryName,visited,edges,sortedNames);});}const mixed=[];sortedNames.forEach(name=>{const sampleName=getSampleName(name,boneCombo);if(sampleName){const sample=samples.get(sampleName);const sampleEntries=mapEntries(sample);sampleEntries.forEach(entry=>{if(entry["name"]===name){mixed.push(entry);}});}});return mixed};const mixSkins=(boneCombo,samples,boneNames,skinNames,slotNames,debug=false)=>{const skins=[];const slotTransform=(slot,sampleName)=>{const transformedSlot={};const sampleBones=samples.get(sampleName).bones;// eslint-disable-next-line no-console
// console.log('sampleBones', sampleBones)
// eslint-disable-next-line @typescript-eslint/no-unused-vars
for(const[name,_attachment]of Object.entries(slot)){const attachment=JSON.parse(JSON.stringify(_attachment));if(attachment.type==="mesh"){const{vertices}=attachment;const{uvs}=attachment;if(vertices.length>uvs.length){const transformedVertices=vertices;let i=0;while(i<vertices.length){const numBones=vertices[i++];for(let j=0;j<numBones;j++){const boneIndex=vertices[i];// eslint-disable-next-line no-console
if(debug){console.log(boneIndex);}const boneName=sampleBones[boneIndex]["name"];if(debug){console.log(boneName);}transformedVertices[i]=boneNames.indexOf(boneName);i+=4;}}attachment.vertices=transformedVertices;}}transformedSlot[name]=attachment;}return transformedSlot};const getSlotInSampleSkins=(skinName,slotName,sampleName)=>{const sample=samples.get(sampleName);const skin=sample.skins.find(skin=>skin["name"]===skinName);if(skin){const skinAttachments=skin.attachments;if(skinAttachments===undefined){return undefined}const slot=skinAttachments[slotName];return slot}return undefined};const comboRoot=boneCombo[0];for(const skinName of skinNames){const skin={};skin.name=skinName;const mixed={};for(const slotName of slotNames){const sampleName=getSampleName(slotName,boneCombo);if(sampleName){const map=getSlotInSampleSkins(skinName,slotName,sampleName);if(map){mixed[slotName]=slotTransform(map,sampleName);continue}}const map2=getSlotInSampleSkins(skinName,slotName,comboRoot);if(map2){mixed[slotName]=slotTransform(map2,sampleName);}}skin.attachments=mixed;skins.push(skin);}return skins};const correctBones=bones=>{const final=[];let invalidBoneOrders=[];const boneTrees={};let src=bones;let fixLoop=0;while(fixLoop<100){for(const bone of src){//@ts-ignore
if(bone.parent){//@ts-ignore
if(bone.parent in boneTrees){//@ts-ignore
boneTrees[bone.name]=bone.parent;final.push(bone);}else {invalidBoneOrders.push(bone);}}else {boneTrees[bone.name]="";final.push(bone);}}if(invalidBoneOrders.length==0){break}fixLoop+=1;src=invalidBoneOrders;invalidBoneOrders=[];}return final};const mix=(boneCombo,samples,sortingRules,debug=false)=>{let bones=mixEntriesFromSample(sample=>sample.bones,boneCombo,samples,sortingRules);bones=correctBones(bones);const boneNames=bones.map(bone=>bone["name"]);const slots=mixEntriesFromSample(sample=>sample.slots,boneCombo,samples,sortingRules);const slotNames=slots.map(slot=>slot["name"]);const ik=mixEntriesFromSample(sample=>sample.ik,boneCombo,samples,sortingRules);const skinNames=["default"];const skins=mixSkins(boneCombo,samples,boneNames,skinNames,slotNames,debug);const events={};for(const[key,sample]of Array.from(samples.entries())){if(sample.events===null){continue}for(const q in sample.events){events[q]={};}}return new MixedSkeletonData(bones,slots,skins,ik,events,null)};

class AxieMierStuff{constructor(){this.initilized=false;}load(samplesJson,variantsJson,animationsJson){this.variantList=variantsJson.items;this.loadSamples(samplesJson);this.loadAnimations(animationsJson);this.initilized=true;}loadSamples(samplesJson){this.sampleSkeletonMap=samplesJson.items;this.sortingRules=samplesJson.sortingRules;this.accessoryAnims=samplesJson.accessoryAnims;}loadAnimations(animationsJson){const samplesData=animationsJson.items;this.animationHeaders=samplesData.header;this.animationCRCMap=samplesData.animations;}generateAsset(adultCombo,skipAnim,debug=false){const boneCombo=[];const samples=new Map;const sampleAnimations={};for(let i=0;i<boneComboTypes.length;i++){boneCombo.push(adultCombo.get(boneComboTypes[i]));}for(const sampleName of boneCombo){if(sampleName in samples){continue}if(sampleName in this.sampleSkeletonMap){const sampleResult=this.sampleSkeletonMap[sampleName];for(const animationName in sampleResult.keyAnimations){if(!(animationName in sampleAnimations)){sampleAnimations[animationName]={};}if(!(sampleName in sampleAnimations[animationName])){sampleAnimations[animationName][sampleName]=sampleResult.keyAnimations[animationName];}}samples.set(sampleName,sampleResult);}}const mixedJson=mix(boneCombo,samples,this.sortingRules,debug);const bones=mixedJson.bones;const slots=mixedJson.slots;const skins=mixedJson.skins;for(const slotName in skins[0].attachments){const slotAttachments=skins[0].attachments[slotName];if(ACESSORY_SLOTS.includes(slotName)){if(adultCombo.has(slotName)){let selectedAccessory=adultCombo.get(slotName);if(selectedAccessory in slotAttachments){slotAttachments[slotName]=slotAttachments[selectedAccessory];}else {selectedAccessory+="_00";if(selectedAccessory in slotAttachments){slotAttachments[slotName]=slotAttachments[selectedAccessory];}}}else {delete skins[0].attachments[slotName];}}if(slotName==="body-class"){const selectedAttachment="body-class-"+adultCombo.get("body-class");const attachmentNames=Object.keys(slotAttachments);for(const attachmentName of attachmentNames){if(attachmentName!=selectedAttachment){delete slotAttachments[attachmentName];}}}else if(slotName.startsWith("body-id-")){if(adultCombo.has("body-id")){let bodyId=adultCombo.get("body-id");if(bodyId.length<=4){bodyId=String(bodyId).padStart((6-bodyId.length)/2+bodyId.length," ");}let val=-1;const i=102-slotName.charCodeAt(8);if(i>=0&&i<bodyId.length&&bodyId.charCodeAt(i)>=48&&bodyId.charCodeAt(i)<=57){val=bodyId.charCodeAt(i)-48;}const selectedAttachment="body-id-"+String(val).padStart(2,"0")+"-"+adultCombo.get("body-class");const attachmentNames=Object.keys(slotAttachments);for(const attachmentName of attachmentNames){if(attachmentName!=selectedAttachment){delete slotAttachments[attachmentName];}}}}}if(skipAnim){return mixedJson}else {const animations={};for(const animationHeader of this.animationHeaders){let crcSampleMaps={};if(animationHeader.name in sampleAnimations){crcSampleMaps=sampleAnimations[animationHeader.name];}if(Object.keys(crcSampleMaps).length===0&&animationHeader.defaultKey===null){continue}const mixedBones={};const mixedSlots={};const mixedDrawOrder=[];for(const bone of bones){const sampleName=getSampleName(bone.name,boneCombo);let crcKey=animationHeader.defaultKey;if(sampleName in crcSampleMaps){crcKey=crcSampleMaps[sampleName];}if(crcKey in this.animationCRCMap){const sampleAnimation=this.animationCRCMap[crcKey];if(sampleAnimation.bones){if(bone.name in sampleAnimation.bones){mixedBones[bone.name]=sampleAnimation.bones[bone.name];}}}}for(const slot of slots){const sampleName=getSampleName(slot.name,boneCombo);let crcKey=animationHeader.defaultKey;if(sampleName in crcSampleMaps){crcKey=crcSampleMaps[sampleName];}if(crcKey in this.animationCRCMap){const sampleAnimation=this.animationCRCMap[crcKey];if(sampleAnimation.slots){if(slot.name in sampleAnimation.slots){mixedSlots[slot.name]=sampleAnimation.slots[slot.name];}}}}//Inject Accessory anim
if(animationHeader.name=="action/idle/normal"||animationHeader.name=="action/idle/random-01"||animationHeader.name=="action/idle/random-02"||animationHeader.name=="action/idle/random-03"||animationHeader.name=="action/idle/random-04"){for(const accessorySlot in this.accessoryAnims){if(adultCombo.has(accessorySlot)){const selectedAccessory=adultCombo.get(accessorySlot);mixedSlots[accessorySlot]=this.accessoryAnims[accessorySlot][selectedAccessory];}}}{const sampleName=boneCombo[0];//body
let crcKey=animationHeader.defaultKey;if(sampleName in crcSampleMaps){crcKey=crcSampleMaps[sampleName];}if(crcKey in this.animationCRCMap){const sampleAnimation=this.animationCRCMap[crcKey];if(sampleAnimation.drawOrder){for(const drawOrderMap in sampleAnimation.drawOrder){const clone={time:drawOrderMap["time"],offsets:[]};const offsets=drawOrderMap["offsets"];if(offsets){for(const offsetMap of offsets){const slotIndex=slots.findIndex(x=>x.name===offsetMap.slot);const bodySlotIndex=slots.findIndex(x=>x.name==="body");if(slotIndex!=-1&&bodySlotIndex!=-1){//phuongnk - move it above body
if(slotIndex<bodySlotIndex){const cloneOffsetData={slot:offsetMap.slot,offset:bodySlotIndex-slotIndex};clone.offsets.push(cloneOffsetData);}}}}mixedDrawOrder.push(clone);}}}}const mixedAnimation={slots:mixedSlots,bones:mixedBones};if(mixedDrawOrder&&mixedDrawOrder.length>0){mixedAnimation["drawOrder"]=mixedDrawOrder;}animations[animationHeader.name]=mixedAnimation;}mixedJson.animations=animations;return mixedJson}}}const mixerStuff=/*#__PURE__*/new AxieMierStuff;

class AxieBuilderResult{// eslint-disable-next-line @typescript-eslint/no-empty-function
constructor(){}}function initAxieMixer(GenesData,SamplesData,VariantsData,AnimationsData){genesStuff.load(GenesData);mixerStuff.load(SamplesData,VariantsData,AnimationsData);}const getAxieSpineFromGenes=(gene,meta,skipAnimation=false)=>{const bodyStructure=getAxieBodyStructure512(gene);const combo=genesStuff.getAdultCombo(bodyStructure);if(meta!=null){meta.forEach((v,k)=>combo.set(k,v));}const variantIdx=genesStuff.getAxieColorsVariant(bodyStructure.primaryColors[0],bodyStructure.bodySkin,bodyStructure.class);return getAxieSpineFromCombo(combo,variantIdx,skipAnimation)};function isNumeric(value){return /^-?\d+$/.test(value)}function overridePartSample(partType,combo){const partTypeStr=partType.toLowerCase();const stageKey=`${partTypeStr}.stage`;const skinKey=`${partTypeStr}.skin`;let partStage=-1;let partSkin=-1;if(combo.has(stageKey)&&isNumeric(combo.get(stageKey))){partStage=parseInt(combo.get(stageKey));}if(combo.has(skinKey)&&isNumeric(combo.get(skinKey))){partSkin=parseInt(combo.get(skinKey));}const finalPartSample=genesStuff.overridePartSample(partType,combo.get(partTypeStr),partStage,partSkin);return finalPartSample}const getAxieSpineFromCombo=(combo,colorVariant,skipAnimation=false)=>{const builderResult=new AxieBuilderResult;if(!mixerStuff.initilized){builderResult.error="Please initAxieMixer first";}else {try{const bodySkinKey="body.skin";if(combo.has(bodySkinKey)&&isNumeric(combo.get(bodySkinKey))){const bodySkin=parseInt(combo.get(bodySkinKey));if(bodySkin==1){combo.set("body","body-frosty");}else if(bodySkin==2){combo.set("body","body-summer");}else if(bodySkin==3){combo.set("body","body-nightmare");}if(bodySkin!=0){colorVariant=genesStuff.getAxieColorsVariant(0,bodySkin,combo.get("body-class"));}}combo.set("back",overridePartSample(exports.AxiePartType.Back,combo));combo.set("ears",overridePartSample(exports.AxiePartType.Ears,combo));combo.set("ear",combo.get("ears"));combo.set("eyes",overridePartSample(exports.AxiePartType.Eyes,combo));combo.set("horn",overridePartSample(exports.AxiePartType.Horn,combo));combo.set("mouth",overridePartSample(exports.AxiePartType.Mouth,combo));combo.set("tail",overridePartSample(exports.AxiePartType.Tail,combo));builderResult.combo=combo;builderResult.variant=genesStuff.getAxieVariantKeyFromIndex(colorVariant);if(combo!=null){combo.forEach((v,k)=>{builderResult.combo.set(k.replace("accessory-","body-"),v.replace("accessory-","body-"));});if(combo.has("accessory-suit-off")){const bodySample=builderResult.combo.get("body").replace("-mystic-","-");builderResult.combo.set("body",bodySample);}// hide nightmare accessory cheek2a for mystic body
const accessoryCheek=combo.get("accessory-cheek");if(accessoryCheek=="accessory-cheek2a"&&builderResult.combo.get("body").includes("-mystic-")){builderResult.combo.delete("body-cheek");}}const jMixed=mixerStuff.generateAsset(builderResult.combo,skipAnimation);if(jMixed==null){builderResult.error="GenerateAsset Failed";}else {if(combo!=null&&combo.has("embedded-color")){const variantHex=colorVariant.toString(16).padStart(2,"0");const partColorShift=genesStuff.getAxieColorPartShift(builderResult.variant);const slots=jMixed.slots;for(const slot of slots){let isPartShift=partColorShift[0]!=="0";for(let i=1;i<boneComboTypes.length;i++){if(slot.name.startsWith(boneComboTypes[i].toString())){isPartShift=partColorShift[i]!=="0";}}const shiftHex=isPartShift?"02":"00";if("color"in slot){slot.color=variantHex+shiftHex+"00"+slot.color.substring(6);}else {slot.color=variantHex+shiftHex+"00ff";}}}builderResult.skeletonDataAsset=jMixed;}}catch(error){builderResult.error=error.message;}}return builderResult};const getVariantAttachmentPath=(slotName,attachmentPath,variantKey,partColorShift)=>{let imagePath=attachmentPath.replace(".","/");if(mixerStuff.variantList.indexOf(attachmentPath)!=-1){let isPartShift=false;if(partColorShift.length==7){isPartShift=partColorShift[0]!=="0";for(let i=1;i<boneComboTypes.length;i++){if(slotName.startsWith(boneComboTypes[i].toString())){isPartShift=partColorShift[i]!=="0";}}}if(isPartShift){imagePath+=`/${variantKey}-shift.png`;}else {imagePath+=`/${variantKey}.png`;}}else {imagePath+=".png";}return imagePath};const getAxieColorPartShift=variant=>{const partColorShift=genesStuff.getAxieColorPartShift(variant);return partColorShift};

exports.ACESSORY_SLOTS = ACESSORY_SLOTS;
exports.AxieBuilderResult = AxieBuilderResult;
exports.axiePartTypes = axiePartTypes;
exports.boneComboTypes = boneComboTypes;
exports.characterClasses = characterClasses;
exports.exportAvatarLayers = exportAvatarLayers;
exports.genesStuff = genesStuff;
exports.getAxieBodyStructure512 = getAxieBodyStructure512;
exports.getAxieColorPartShift = getAxieColorPartShift;
exports.getAxieSpineFromCombo = getAxieSpineFromCombo;
exports.getAxieSpineFromGenes = getAxieSpineFromGenes;
exports.getCharacterClassFromValue = getCharacterClassFromValue;
exports.getVariantAttachmentPath = getVariantAttachmentPath;
exports.initAxieMixer = initAxieMixer;
exports.mixerStuff = mixerStuff;
